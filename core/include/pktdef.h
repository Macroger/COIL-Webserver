...
namespace coil::protocol
{
	...
	private:
		...
		coil::protocol::Endianness endianness; // Endianness of the packet
	...
	public:
		PktDef();
		PktDef(const char*, int, coil::protocol::Endianness endianness = coil::protocol::Endianness::LittleEndian);
		~PktDef();  // Destructor
		...
		void DumpHex(const char* label, const char* data, int len);
		void ParseHeaderBytes(const uint8_t in[4], Header& hdrOut, Endianness wireOrder) noexcept;
		void BuildHeaderBytes(const Header& hdr, Endianness wireOrder, uint8_t out[4]) noexcept;
		void WriteUint16ToBytes(uint8_t* bytes, uint16_t value, Endianness order) noexcept;
		uint16_t ReadUint16FromBytes(const uint8_t* bytes, Endianness order) noexcept;
	};
}